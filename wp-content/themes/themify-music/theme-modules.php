 ...
 * @return string
 */
function themify_social_share( $data = array() ) {
	/**
	 * Variable key in theme settings
	 * @var string
	 */
	...
	$html = sprintf('<p><label for="%1$s"><input type="checkbox" id="%1$s" name="%1$s" %2$s /> %3$s</label></p>',
		$key,
		checked( themify_get( $key ), 'on', false ),
		__('Disable social share in archive views (eg. category, tag pages, etc.)', 'themify')
	);
	...
	$html .= sprintf('<p><label for="%1$s"><input type="checkbox" id="%1$s" name="%1$s" %2$s /> %3$s</label></p>',
		$key,
		checked( themify_get( $key ), 'on', false ),
		__('Disable social share in single post view', 'themify')
	);
